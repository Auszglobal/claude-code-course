...
<p>1. <code>/loop 5m "check tests"</code> 和 <code>/loop "check tests"</code> 有什麼區別？</p>
<label><input type="radio" name="q1" value="0"> 沒有區別，兩者完全一樣</label>
<label><input type="radio" name="q1" value="1"> 第一個每 5 分鐘固定檢查一次，第二個讓 Claude 自己決定間隔</label>
<label><input type="radio" name="q1" value="2"> 第一個只檢查 5 次，第二個無限檢查</label>
<label><input type="radio" name="q1" value="3"> 第一個用快取，第二個不用快取</label>
<div class="quiz-explain">加上時間間隔（如 5m）使用固定間隔模式，Claude 嚴格按照指定時間重複執行。省略時間間隔則進入動態自主模式，Claude 會使用 ScheduleWakeup 自行決定最佳的喚醒時間。</div>
</div>

<div class="quiz-q" data-answer="1">
<p>2. 為什麼 Claude 在動態模式下傾向於在 270 秒以內安排下一次喚醒？</p>
<label><input type="radio" name="q2" value="0"> 因為 270 秒是 Claude 的最大運行時間</label>
<label><input type="radio" name="q2" value="1"> 因為超過 270 秒後快取會開始失效，超過 300 秒快取完全消失，導致需要重新讀取所有資料</label>
<label><input type="radio" name="q2" value="2"> 因為 270 秒是系統規定的最短間隔</label>
<label><input type="radio" name="q2" value="3"> 因為 270 秒剛好等於一次完整的 build 時間</label>
<div class="quiz-explain">Claude 的 prompt cache 在 270 秒以內保持有效（cache hit），超過 300 秒就會完全失效（cache miss）。在快取命中的情況下，Claude 可以快速繼續之前的工作，不需要重新讀取檔案和對話歷史，這樣更快也更省成本。</div>
</div>

<div class="quiz-q" data-answer="2">
<p>3. 以下哪一個是 <code>/loop</code> 無法做到的？</p>
<label><input type="radio" name="q3" value="0"> 每 10 分鐘自動執行 npm test 並回報結果</label>
<label><input type="radio" name="q3" value="1"> 監控 build 過程並在失敗時自動修復程式碼</label>
<label><input type="radio" name="q3" value="2"> 繞過 settings.json 中設定的安全限制來刪除受保護的檔案</label>
<label><input type="radio" name="q3" value="3"> 搭配 /portal-status Skill 進行定期健康檢查</label>
<div class="quiz-explain">/loop 不會讓 Claude 獲得額外的權限。所有在 settings.json 中設定的 hooks 和安全限制仍然有效。/loop 只是讓原本允許的操作可以自動重複執行，它不能繞過任何安全邊界。</div>
</div>

<div class="quiz-q" data-answer="0">
<p>4. 在自主 Agent 模式中，Claude 的運作循環順序是什麼？</p>
<label><input type="radio" name="q4" value="0"> 觀察 → 判斷 → 執行 → 回報 → 安排下次喚醒</label>
<label><input type="radio" name="q4" value="1"> 執行 → 觀察 → 回報 → 判斷 → 等待</label>
<label><input type="radio" name="q4" value="2"> 回報 → 執行 → 觀察 → 判斷 → 休眠</label>
<label><input type="radio" name="q4" value="3"> 判斷 → 回報 → 觀察 → 執行 → 結束</label>
<div class="quiz-explain">自主 Agent 模式遵循：觀察（檢查當前狀態）→ 判斷（決定是否需要行動）→ 執行（修復問題或執行指令）→ 回報（告訴你做了什麼）→ 安排下次喚醒（重複循環）。</div>
</div>

<div class="quiz-q" data-answer="3">
<p>5. 你想讓 Claude 每 30 分鐘自動執行一個自訂的 <code>/portal-status</code> Skill。正確的語法是什麼？</p>
<label><input type="radio" name="q5" value="0"> <code>/loop portal-status 30m</code></label>
<label><input type="radio" name="q5" value="1"> <code>/schedule 30m /portal-status</code></label>
<label><input type="radio" name="q5" value="2"> <code>/portal-status --repeat 30m</code></label>
<label><input type="radio" name="q5" value="3"> <code>/loop 30m /portal-status</code></label>
<div class="quiz-explain">正確語法是 <code>/loop 時間間隔 任務</code>。<code>/loop 30m /portal-status</code> 表示每 30 分鐘執行一次 /portal-status Skill。時間間隔放在任務前面。</div>
</div>

<div class="quiz-q" data-answer="1">
<p>6. 如何停止一個正在運行的 <code>/loop</code>？</p>
<label><input type="radio" name="q6" value="0"> 關閉終端機視窗</label>
<label><input type="radio" name="q6" value="1"> 按 Esc 鍵</label>
<label><input type="radio" name="q6" value="2"> 輸入 <code>/stop</code></label>
<label><input type="radio" name="q6" value="3"> 等待它自動結束</label>
<div class="quiz-explain">按 Esc 鍵是停止 /loop 的標準方式。Claude 會在下一次檢查點之前停止循環。雖然關閉終端也能停止，但 Esc 是更優雅的方式，不會中斷其他工作。</div>
</div>

<div class="quiz-q" data-answer="2">
<p>7. <code>/loop</code> 和 <code>/schedule</code> 的主要區別是什麼？</p>
<label><input type="radio" name="q7" value="0"> /loop 只能用固定間隔，/schedule 可以用動態間隔</label>
<label><input type="radio" name="q7" value="1"> /loop 只能在 Mac 上使用，/schedule 只能在 Windows 上使用</label>
<label><input type="radio" name="q7" value="2"> /loop 在你的電腦上本地運行，/schedule 在雲端運行（即使電腦關機也能執行）</label>
<label><input type="radio" name="q7" value="3"> 兩者完全相同，只是名稱不同</label>
<div class="quiz-explain">/loop 在你本地的 Claude Code 終端中運行，需要保持終端開啟。/schedule 是排程代理，部署到雲端，按照 cron 時間表自動執行，即使你的電腦關機也會按時運行。</div>
</div>

<div class="quiz-q" data-answer="0">
<p>8. 在動態模式的範例中，Claude 在 build 剛開始時設定 60 秒後再檢查，但在發現並修復 bug 後設定 30 秒再檢查。這反映了什麼策略？</p>
<label><input type="radio" name="q8" value="0"> Claude 根據任務緊急程度動態調整檢查頻率 — 修復後需要更頻繁地確認結果</label>
<label><input type="radio" name="q8" value="1"> 30 秒是系統強制的最小間隔</label>
<label><input type="radio" name="q8" value="2"> Claude 隨機選擇檢查間隔</label>
<label><input type="radio" name="q8" value="3"> 60 秒和 30 秒沒有實質區別</label>
<div class="quiz-explain">Claude 在動態模式下會智慧地調整檢查頻率。Build 剛開始時不急（60 秒），但修完 bug 後需要快速確認修復是否有效（30 秒）。這種基於情境的判斷讓自主 Agent 既高效又不浪費資源。</div>
</div>
